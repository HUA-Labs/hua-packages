...
hua allows extending framework functionality through plugins. Plugins register components, hooks, and config schemas into the registry.
...
interface HuaPlugin {
  ...
  init?(config: HuaConfig): void | Promise<void>  // Initialization function
...
```typescript
function registerPlugin(plugin: HuaPlugin): void
```
...
```typescript
function getPlugin(name: string): HuaPlugin | undefined
```

### getAllPlugins

```typescript
function getAllPlugins(): HuaPlugin[]
```
...
```typescript
import { pluginRegistry } from '@hua-labs/hua/framework'
...
```typescript
import type { HuaPlugin } from '@hua-labs/hua/framework/shared'
