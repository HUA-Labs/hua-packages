...
hua fully supports Next.js server components and SSR. Correct entry point usage is essential.

## Server Entry (`framework/server`)

`@hua-labs/hua/framework/server` provides two server-only functions:
...
Dynamically loads `hua.config.ts` (or `.js`, `.mjs`) from the filesystem.

```typescript
function loadConfig(): HuaConfig
```

**Search order:** `hua.config.ts` -> `.js` -> `.mjs` -> product preset defaults
...
<HuaProvider config={{ ...config, i18n: { ...config.i18n, initialTranslations } }}>
  {children}
</HuaProvider>
```
