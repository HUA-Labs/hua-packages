...
export function ClientLayout({ translations, children }) {
  return <HuaProvider config={{ i18n: { initialTranslations: translations } }}>
    {children}
  </HuaProvider>
}
```
...
import type { HuaConfig, GEOConfig } from '@hua-labs/hua/framework/shared'
```
...
| `HuaProvider` | 'use client' (cannot render React components in Edge) |
...
Solution: Use within HuaProvider or UnifiedProviders
```
