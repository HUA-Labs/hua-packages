...
is empty, so the validator admits either that exact omission or a complete
`users`/`teams`/`apps` object whose three arrays are empty. An explicit null,
partial object, non-empty actor array, API error, or otherwise incomplete
response never becomes an empty allowlist. For an exact
