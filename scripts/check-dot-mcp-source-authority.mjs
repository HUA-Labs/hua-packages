...
const defaultConfig = resolve(
  repositoryRoot,
  "config/dot-mcp-source-authority.json",
);
const PACKAGE_PATH = "packages/hua-dot-mcp";
const PACKAGE_PREFIX = `${PACKAGE_PATH}/`;
const SCHEMA = "hua-dot-mcp-source-authority.v1";
const AUTHORITY_KIND = "platform-dot-mcp-package-projection";
...
const REVIEWED_ARTIFACT_ROSTER_DIGEST =
  "8860fcd2670ea1cebbcdff05d29ab83457322ef62a1009d392898ae2fc0d5a4f";
...
const REQUIRED_TARGETS = Object.freeze(["package/dist/index.js"]);
...
function expectedKind(path) {
  ...
  if (path === `${PACKAGE_PATH}/vitest.config.ts`) return "build";
...
function main() {
  ...
  if (options.json) console.log(canonicalJson(result).trimEnd());
    ...
    console.log(
      `PASS Dot MCP source authority: ${result.rows} rows, map ${result.mapDigest}, sourceVerified=${result.sourceVerified}, artifactVerified=${result.artifactVerified}`,
    );
  }
}
...
try {
  ...
  console.error(
    `FAIL Dot MCP source authority: ${code}${detail ? ` (${detail})` : ""}`,
  );
  process.exitCode = 1;
}
