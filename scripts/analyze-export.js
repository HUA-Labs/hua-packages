...
function printUsage() {
  console.log(`Usage: node scripts/analyze-export.js <analysis-export.json>

Options:
  <analysis-export.json>       Path to the export file to analyze
  ANALYZE_EXPORT_PATH          Environment fallback when no CLI path is passed
...
function resolveInputPath() {
  ...
  const inputPath = arg || process.env.ANALYZE_EXPORT_PATH;
  if (!inputPath) {
    printUsage();
    console.error(
      "Error: pass an export JSON file path or set ANALYZE_EXPORT_PATH.",
    );
    process.exit(1);
  }
  ...
  if (!fs.existsSync(resolvedPath)) {
    console.error(`Error: export file does not exist: ${resolvedPath}`);
    process.exit(1);
  }
  if (!fs.statSync(resolvedPath).isFile()) {
    console.error(`Error: export path is not a file: ${resolvedPath}`);
    process.exit(1);
  }
  return resolvedPath;
}
...
function loadExportData(filePath) {
  ...
  try {
    ...
    console.error(`Error: failed to parse export JSON: ${message}`);
    process.exit(1);
  }

  if (!data || !Array.isArray(data.data)) {
    console.error("Error: export JSON must contain a data array.");
    process.exit(1);
  }
  return data;
}

const data = loadExportData(resolveInputPath());
...
const slipCounts = {};
entries.forEach((e) => {
  slipCounts[e.slip || "null"] = (slipCounts[e.slip || "null"] || 0) + 1;
});
...
const langs = {};
entries.forEach((e) => {
  langs[e.user_language] = (langs[e.user_language] || 0) + 1;
});
console.log("Language:", JSON.stringify(langs));

const models = {};
entries.forEach((e) => {
  models[e.model_name || "null"] = (models[e.model_name || "null"] || 0) + 1;
});
...
if (slipSoft.length > 0) {
  ...
  slipSoft.forEach((e, i) => {
    console.log(
      `  [${i + 1}] sentiment:${e.sentiment_score} a:${e.affect_tier} m:${e.momentum_tier} ethics:${e.ethics} date:${e.created_at.split("T")[0]}`,
    );
  });
}

const r2 = entries.filter(
  (e) => e.affect_tier >= 4.5 && e.momentum_tier >= 4.5,
);
console.log("\n=== R2 candidates (a>=4.5 & m>=4.5) ===");
console.log("Count:", r2.length);
r2.forEach((e, i) => {
  console.log(
    `  [${i + 1}] slip:${e.slip} sent:${e.sentiment_score} a:${e.affect_tier} m:${e.momentum_tier} ethics:${e.ethics}`,
  );
});
...
sortedTags.slice(0, 25).forEach(([tag, count]) => {
  console.log(
    `  ${tag}: ${count} (${((count / entries.length) * 100).toFixed(1)}%)`,
  );
});
...
console.log(
  `N:${sents.length} Mean:${mean.toFixed(1)} Median:${median} Min:${Math.min(...sents)} Max:${Math.max(...sents)}`,
);

// Soft vs None sentiment comparison
if (slipSoft.length > 0 && slipNone.length > 0) {
  const softSent = slipSoft
    .map((e) => e.sentiment_score)
    .filter((x) => x != null);
  const noneSent = slipNone
    .map((e) => e.sentiment_score)
    .filter((x) => x != null);
  ...
  console.log(
    `None sentiment: M=${noneMean.toFixed(1)} (n=${noneSent.length})`,
  );
}
...
console.log(`\n=== Tier ranges ===`);
console.log(
  `tier_a: ${Math.min(...tAs)}-${Math.max(...tAs)} (mean: ${(tAs.reduce((a, b) => a + b, 0) / tAs.length).toFixed(2)})`,
);
console.log(
  `tier_m: ${Math.min(...tMs)}-${Math.max(...tMs)} (mean: ${(tMs.reduce((a, b) => a + b, 0) / tMs.length).toFixed(2)})`,
);
