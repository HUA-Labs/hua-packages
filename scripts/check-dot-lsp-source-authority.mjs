...
const defaultConfig = resolve(
  repositoryRoot,
  "config/dot-lsp-source-authority.json",
);
...
const REVIEWED_ARTIFACT_ROSTER_DIGEST =
  "bc3e4ad92c4922fd5b6677bfd5cf74c4768c0b2abf45a44ea21423bb4595ce62";
...
function main() {
  ...
  if (options.json) console.log(canonicalJson(result).trimEnd());
    ...
    console.log(
      `PASS Dot LSP source authority: ${result.rows} rows, map ${result.mapDigest}, sourceVerified=${result.sourceVerified}, artifactVerified=${result.artifactVerified}`,
    );
  }
}
...
try {
  ...
  console.error(
    `FAIL Dot LSP source authority: ${code}${detail ? ` (${detail})` : ""}`,
  );
  process.exitCode = 1;
}
