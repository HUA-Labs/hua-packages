...
import { DateFormatterReturn, DateFormatterOptions, LocaleDateFormatterOptions, RelativeTimeOptions } from '../types';
import {
  formatDate,
  formatDateTime,
  formatDateLocalized,
...
export function useDateFormatter(): DateFormatterReturn {
  ...
  // 로케일 기반 날짜 포맷팅 함수
  const formatDateLocalizedFn = useMemo(() => {
    return (date: Date, options?: LocaleDateFormatterOptions) => {
      return formatDateLocalized(date, locale, options);
    };
  }, [locale]);

  // 로케일 기반 날짜+시간 포맷팅 함수
  const formatDateTimeLocalizedFn = useMemo(() => {
    return (date: Date, options?: LocaleDateFormatterOptions) => {
      return formatDateTimeLocalized(date, locale, options);
    };
  }, [locale]);
  ...
  return {
    ...
    formatDateLocalized: formatDateLocalizedFn,
    formatDateTimeLocalized: formatDateTimeLocalizedFn,
