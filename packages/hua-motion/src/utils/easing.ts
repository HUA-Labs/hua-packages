...
export function getEasing(easingName: unknown): EasingFunction {
  ...
  if (typeof easingName === 'string') {
    ...
    if (fallback && fallback in easing) {
      console.warn(`[HUA Animation] Unknown easing "${easingName}", using fallback "${fallback}".`)
      return easing[fallback]
    }

    console.warn(`[HUA Animation] Unknown easing "${easingName}", using default "easeOut".`)
  }
...
export function safeApplyEasing(t: number, easingName: unknown): number {
  try {
    ...
    console.error(`[HUA Animation] Failed to apply easing "${easingName}":`, err)
    return easeOut(t)
  }
}
