import { AnimationState, AnimationElement } from '../types'

/**
 * AnimationStateManager - 애니메이션 상태 관리 클래스
 * 
...
export class AnimationStateManager {
  ...
  private listeners: Map<string, Set<(state: AnimationState) => void>> = new Map()

  /**
   * 요소의 상태를 초기화
   */
  initializeElement(elementId: string, config: AnimationElement): void {
   ...
   * 애니메이션 값 업데이트
   */
  updateAnimationValues(elementId: string, values: Partial<AnimationState>): void {
    const state = this.states.get(elementId)
    if (!state) return
   ...
   * 현재 상태 조회
   */
  getState(elementId: string): AnimationState | undefined {
   ...
   * 상태 변경 리스너 등록
   */
  subscribe(elementId: string, listener: (state: AnimationState) => void): () => void {
    ...
    // 구독 해제 함수 반환
    return () => {
      this.listeners.get(elementId)?.delete(listener)
    }
  }
   ...
   * 리스너들에게 상태 변경 알림
   */
  notifyListeners(elementId: string, state: AnimationState): void {
    this.listeners.get(elementId)?.forEach(listener => {
      try {
        listener(state)
      } catch (error) {
        console.error(`AnimationStateManager listener error for ${elementId}:`, error)
      }
    })
  }
   ...
   */
  debug(): void {
    console.log('AnimationStateManager Debug:')
    this.states.forEach((state, elementId) => {
      console.log(`${elementId}:`, {
        internal: state.internalVisibility,
        triggered: state.triggeredVisibility,
        final: state.finalVisibility,
