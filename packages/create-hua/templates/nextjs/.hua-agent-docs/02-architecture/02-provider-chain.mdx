...
hua auto-composes multiple Context Providers via `UnifiedProviders`. No manual provider nesting required.

## Provider Order

```
<BrandingProvider>           <- 1. Branding CSS variables (outermost)
  <MotionConfigProvider>     <- 2. Motion configuration
    <ToastProvider>          <- 3. Toast notifications
      <IconProvider>         <- 4. Icon defaults
        <I18nProvider>       <- 5. Internationalization (innermost)
          {children}
        </I18nProvider>
      </IconProvider>
    </ToastProvider>
...
- MotionConfigProvider wraps all UI so components can access motion presets via `useComponentMotion()`.
...
function createProviders(config: HuaConfig) {
  ...
  // 4. I18nProvider (via createI18nStore + createZustandI18n)
  if (config.i18n) {
    const i18nStore = config.i18n.languageStore ?? createI18nStore({
...
HuaProvider = UnifiedProviders + extras:

```tsx
function HuaProvider({ children, config }) {
  return (
...
export default function Layout({ children }) {
  return <HuaProvider config={config}>{children}</HuaProvider>
}
```
...
```tsx
import { UnifiedProviders, getConfig } from '@hua-labs/hua/framework'
