...
+-- @hua-labs/i18n-formatters -> Currency, date, number formatters
...
The main entry (`@hua-labs/hua`) uses **named exports** from all sub-packages. `export *` is avoided to prevent naming conflicts and Turbopack CJS warnings:

```typescript
// Main index.ts structure — named exports only
export { Button as UIButton, Action, Input, Modal, ... } from '@hua-labs/ui'
export { useFadeIn, useSlideUp, useScrollReveal, Motion, ... } from '@hua-labs/motion-core'
export { createCoreI18n, useTranslation, I18nProvider, ... } from '@hua-labs/i18n-core'
export { createZustandI18n, useZustandI18n, ... } from '@hua-labs/i18n-core-zustand'
export { createHuaStore, createI18nStore, ... } from '@hua-labs/state'
// ... other packages
```
...
- **HuaProvider**: Layout + theme CSS injection + language transition animation
...
- **Error**: ErrorBoundary, HuaPage error boundaries
...
HuaProvider
  ...
  +-- LanguageTransitionWrapper -> Fade on language switch
      +-- {children}
          +-- HuaPage -> Motion + error boundary + vibe
              +-- Actual page content
```
...
export default function RootLayout({ children }) {
  return (
    <html><body>
      <HuaProvider config={config}>{children}</HuaProvider>
    </body></html>
  )
}
```
