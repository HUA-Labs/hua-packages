...
description: ErrorBoundary, HuaPage error boundaries, custom fallback
category: systems
related:
  - ./05-loading.mdx
...
export default function DiaryPage() {
  return (
    <HuaPage title="Write Diary" vibe="clean">
      <DiaryEditor />
    </HuaPage>
  )
}
...
<HuaPage enableErrorBoundary={false}>
  <SimpleContent />
</HuaPage>
```
...
```tsx
// Error handling for a specific section without HuaPage
<div>
  <Header />
  <ErrorBoundary
...
- The `seo` prop of `HuaPage` is currently unused and only logs a warning message in dev. Use the GEO system instead.
