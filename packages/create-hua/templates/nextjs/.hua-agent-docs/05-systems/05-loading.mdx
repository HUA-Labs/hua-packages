...
hua provides components and hooks for consistent loading UI. Fast responses appear seamlessly without a loading indicator, while slow responses provide appropriate feedback.
...
```tsx
import { SuspenseWrapper } from '@hua-labs/hua/framework'
...
interface SkeletonGroupProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  className?: string              // legacy; prefer dot prop on child Skeletons
}
```
...
```tsx
import { SkeletonGroup, Skeleton } from '@hua-labs/hua/framework'

<SkeletonGroup spacing="md">
  <Skeleton dot="h-8 w-48" />    {/* Title */}
  <Skeleton dot="h-4 w-full" />  {/* Body line 1 */}
  <Skeleton dot="h-4 w-3/4" />   {/* Body line 2 */}
</SkeletonGroup>
```
...
```tsx
import { useLoadingState } from '@hua-labs/hua/framework'
```
...
```tsx
import { withSuspense } from '@hua-labs/hua/framework'

const LazyChart = withSuspense(
  React.lazy(() => import('./Chart')),
  <Skeleton dot="h-64 w-full" />
)
...
```tsx
import { SuspenseWrapper, SkeletonGroup, Skeleton } from '@hua-labs/hua/framework'

function PageSkeleton() {
  return (
    <SkeletonGroup spacing="lg">
      <Skeleton dot="h-10 w-64" />
      <Skeleton dot="h-4 w-full" />
      <Skeleton dot="h-4 w-full" />
      <Skeleton dot="h-4 w-3/4" />
      <Skeleton dot="h-48 w-full" />
    </SkeletonGroup>
  )
}
...
function UserProfile({ userId }) {
  ...
  if (showSkeleton) {
    return <SkeletonGroup>
      <Skeleton dot="h-12 w-12 rounded-full" />
      <Skeleton dot="h-4 w-32" />
    </SkeletonGroup>
  }
...
<div style={dot('grid grid-cols-2 gap-4')}>
  <SuspenseWrapper>
    <AsyncStats />
  </SuspenseWrapper>
  <SuspenseWrapper>
