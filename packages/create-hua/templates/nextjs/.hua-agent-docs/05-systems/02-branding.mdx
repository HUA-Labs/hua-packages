...
description: BrandingProvider, CSS variable generation, styling integration, white-labeling patterns
category: systems
related:
...
> When using `HuaProvider`, `BrandingProvider` is automatically added if `config.branding` is present.
...
> **Note (dot-based projects):** If your project uses the `dot` style engine instead of Tailwind, CSS variables work directly without this config. `generateTailwindConfig` is only needed for legacy Tailwind setups where you want brand tokens available as Tailwind utility classes.
...
**Color mapping:**
| variant | Applied style (inline, via CSS vars) |
|---------|--------------------------------------|
| default | `backgroundColor: var(--color-primary)`, `color: white`, opacity on hover |
| secondary | `backgroundColor: var(--color-secondary)`, `color: white`, opacity on hover |
| outline | `borderColor: var(--color-accent)`, `color: var(--color-accent)`, bg tint on hover |

> Internally uses inline `style` props with CSS variables — no Tailwind dependency.
...
function CustomComponent() {
  return (
    <div style={{ backgroundColor: 'var(--color-primary)', color: 'white', ...dot('p-4') }}>
...
> CSS variables are resolved at runtime, so they cannot be passed as dot tokens. Spread `dot()` for spacing/layout and set CSS var properties explicitly via `style`.
