...
function parseAiContextOptions(): {
  ...
  if (args.includes("--no-agent-docs")) flags["--no-agent-docs"] = true;
  ...
  if (!projectName) {
    try {
      ...
      if (!name) {
        ...
        console.error(
          "Usage: npx create-hua <project-name> [--no-cursor-rules] [--no-agents-md] [--no-skills-md] [--claude-skills] [--lang ko|en|both] [--dry-run] [--install] [--non-interactive] [--english-only] [--skip-version-check]",
        );
        process.exit(1);
      }
