...
`defineConfig` is the core function for defining hua-ux configuration. It handles preset merging, validation, and license/plugin initialization in one call.

## API

```typescript
function defineConfig(config: Partial<HuaUxConfig>): HuaUxConfig
```
...
```typescript
function getConfig(): HuaUxConfig
```
...
```typescript
function setConfig(config: HuaUxConfig): void
```
...
```typescript
import { setConfig, resetConfig, defineConfig } from '@hua-labs/hua-ux/framework/config'
