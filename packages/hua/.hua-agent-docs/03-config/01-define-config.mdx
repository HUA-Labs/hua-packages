...
7. Return the validated config (stateless — does not cache)

> **Note**: `defineConfig` is stateless and does not cache. Only `getConfig()` caches. Assign the return value to a variable and pass it to `HuaProvider` explicitly.
...
export default defineConfig({
  ...
  license: { type: 'enterprise', apiKey: process.env.HUA_LICENSE_KEY }
})
```
...
- `defineConfig` is stateless — it does not write to any cache. Assign the return value and pass it explicitly.
- `getConfig()` is what caches. It returns the last value set via `setConfig()`, or falls back to product preset defaults.
- Calling `getConfig()` without `setConfig` (or without `UnifiedProviders` which calls it internally) returns product preset defaults — no error thrown.
