...
The hua config system works in both client and server, but some functions are environment-specific.
...
import config from '../hua.config'          // Static import
...
```tsx
'use client'
import { getConfig } from '@hua-labs/hua/framework/config'
