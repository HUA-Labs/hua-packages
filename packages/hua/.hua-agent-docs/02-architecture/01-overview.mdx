...
description: hua-ux meta-framework layer structure and package integration
...
hua-ux is a **meta-framework** that unifies individual packages into a single integrated DX.

## Layer Structure

```
+---------------------------------------------+
|              @hua-labs/hua-ux                |  <- Meta-framework
|  +----------------------------------------+  |
...
The main entry (`@hua-labs/hua-ux`) re-exports everything from all sub-packages:
...
- hua-ux is optimized for Next.js. For other React frameworks, use sub-packages (hua-ui, motion-core, etc.) directly.
