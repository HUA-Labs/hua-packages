...
> **Import note**: `metaToObject` and `openGraphToObject` are internal helpers used by the server helpers below. They are NOT exported from `framework/shared`. Use `generateGEOForMetadata` instead for Next.js Metadata integration.
...
- `generateCodeLD`, `generateVideoLD`, `generateOrganizationLD`, `validateJsonLd`, `formatValidationResult`, and `GEO_PRESETS` exist in `geo/index.ts` internally but are NOT exported from `framework/shared`.
