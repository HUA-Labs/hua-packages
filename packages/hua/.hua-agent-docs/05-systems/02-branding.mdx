...
The hua-ux branding system supports **CSS variable-based white-labeling**. Once configured, all UI components automatically use the brand colors and fonts.
...
2. Injects a `<style id="hua-ux-branding-vars">` tag into the DOM
...
```typescript
function useBranding(): HuaUxConfig['branding'] | null
```
...
```typescript
// tailwind.config.ts
import { generateTailwindConfig } from '@hua-labs/hua-ux/framework/shared'
...
- Both HuaUxLayout's ThemeInjector and BrandingProvider inject CSS variables. BrandingProvider's variables override ThemeInjector's default theme.
