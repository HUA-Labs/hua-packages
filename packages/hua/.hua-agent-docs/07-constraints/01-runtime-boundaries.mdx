...
hua-ux operates across three Next.js runtime environments (Server, Client, Edge). Understanding the constraints of each environment is essential.
...
| `loadConfig` | Loads hua-ux.config.ts via require/eval |
...
| `loadConfig` | Static import (`import config from '../hua-ux.config'`) |
...
import type { HuaUxConfig, GEOConfig } from '@hua-labs/hua-ux/framework/shared'
```
...
hua-ux requires the following packages as peer dependencies:
