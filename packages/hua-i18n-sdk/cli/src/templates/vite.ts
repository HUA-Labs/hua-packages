...
export async function createViteConfig(answers: any, cwd: string): Promise<void> {
  // i18n 설정 파일 생성
...
`;
  ...
  // main.tsx 업데이트
  await updateMainTsx(cwd);
  
  // Provider 컴포넌트 생성
  await createProviderComponent(cwd);
}

async function updateMainTsx(cwd: string): Promise<void> {
...
`;
  ...
  await fs.writeFile(path.join(cwd, 'src', 'main.tsx'), mainContent);
