...
export class Translator implements TranslatorInterface {
  ...
  translate(key: string, language?: string): string {
    ...
    try {
      // beginner 모드 감지: namespaces가 ['common']이고 특별한 missingKeyHandler를 사용하는 경우
      const isBeginnerMode = this.config.namespaces && 
        this.config.namespaces.length === 1 && 
        this.config.namespaces[0] === 'common' &&
        this.config.missingKeyHandler &&
        this.config.missingKeyHandler('test', 'ko', 'common').includes('[MISSING:');

      if (isBeginnerMode) {
        return this.findInNamespace('common', key, targetLanguage);
      }
