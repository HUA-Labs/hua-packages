...
describe("Text", () => {
  ...
  it("should merge style override", () => {
    const { container } = render(
      <Text dot="text-lg" style={{ color: "blue" }}>
        Content
      </Text>,
    );
    const el = container.firstChild as HTMLElement;
  ...
  it("keeps generated and opaque raw classes while unstyled removes tag defaults", () => {
    render(
      <Text
        as="h1"
        unstyled
        classDot="hover:text-lg"
        className="raw__text--hash"
        dot="font-bold"
        style={{ color: "purple" }}
      >
        Heading
    ...
    expect(heading.className).toContain("dot-");
    expect(heading).toHaveClass("raw__text--hash");
    expect(heading.style.lineHeight).toBe("");
    expect(heading.style.letterSpacing).toBe("");
    expect(heading.style.fontWeight).toBe("700");
    expect(heading.style.color).toBe("purple");
    expect(heading).not.toHaveAttribute("unstyled");
  });
});
