...
describe("Box", () => {
  ...
  it("should pass through HTML attributes", () => {
    render(
      <Box data-testid="my-box" aria-label="box">
        Content
      </Box>,
    );
  ...
  it("preserves generated and opaque raw classes on the root", () => {
    render(
      <Box classDot="hover:p-4" className="raw__box--hash raw-custom">
        Content
      </Box>,
    );
    const el = screen.getByText("Content");
    expect(el.className).toContain("dot-");
    expect(el).toHaveClass("raw__box--hash", "raw-custom");
    expect(el.className.match(/raw__box--hash/g)).toHaveLength(1);
  });
});
