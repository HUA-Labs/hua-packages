...
export const buttonVariantStyles = dotVariants({
  ...
  variants: {
    variant: {
      ...
      outline: 'border-2 border-[var(--color-border)] bg-transparent text-[var(--color-foreground)]',
...
export const VARIANT_HOVER: Record<string, CSSProperties> = {
  ...
  outline: { backgroundColor: 'var(--color-accent)', color: 'var(--color-accent-foreground)' },
  secondary: { opacity: 0.8 },
  ghost: { backgroundColor: 'var(--color-accent)', color: 'var(--color-accent-foreground)' },
  link: { opacity: 0.8 },
  gradient: { boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -4px rgba(0,0,0,0.1)' },
  neon: { boxShadow: '0 10px 15px -3px rgba(20, 184, 166, 0.4)', borderColor: '#2dd4bf' },
...
export const HOVER_TRANSITIONS: Record<HoverEffect, string> = {
  ...
  scale: 'transform 150ms ease-out, box-shadow 200ms ease-out, opacity 200ms ease-out, background-color 200ms ease-out, color 200ms ease-out',
...
export const HOVER_STYLES: Record<HoverEffect, CSSProperties> = {
  ...
  glow: { boxShadow: '0 10px 15px -3px color-mix(in srgb, var(--color-primary) 25%, transparent)' },
...
const FOCUS_RING_DESTRUCTIVE: CSSProperties = {
  outline: 'none',
  boxShadow: '0 0 0 2px var(--color-background), 0 0 0 3px var(--color-destructive)',
}

export function getFocusRing(variant: string): CSSProperties {
  if (variant === 'destructive') return FOCUS_RING_DESTRUCTIVE
  if (variant === 'outline' || variant === 'ghost' || variant === 'link') return FOCUS_RING_NO_OFFSET
  return FOCUS_RING
}
