...
const Alert = React.forwardRef<HTMLDivElement, AlertProps>(
  ({
    ...
    const iconWrapStyle = useMemo(() => mergeStyles(
      { flexShrink: 0, marginTop: '2px' } as React.CSSProperties,
      VARIANT_ICON[variant],
    ), [variant])

    const closeButtonStyle = useMemo((): React.CSSProperties => mergeStyles(
      {
        display: 'inline-flex',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {title && (
              <h4 style={{ margin: 0, marginBottom: '4px', fontSize: '0.875rem', fontWeight: 600, lineHeight: '1.25rem' }}>
                {title}
              </h4>
            )}
            {description && (
              <p style={{ margin: 0, fontSize: '0.875rem', lineHeight: '1.625' }}>
                {description}
              </p>
            )}
            {children && (
