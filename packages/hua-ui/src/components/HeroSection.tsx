...
const PARTICLES_LAYER: React.CSSProperties = {
  position: "absolute",
  inset: 0,
  background: "linear-gradient(to bottom right, color-mix(in srgb, var(--color-secondary) 50%, transparent), var(--color-background), color-mix(in srgb, var(--color-secondary) 30%, transparent))",
}
 ...
 */
export interface HeroSectionProps extends Omit<React.HTMLAttributes<HTMLElement>, 'className'> {
  // 단일 모드 props
  title?: string
  subtitle?: string
  description?: string
...
const HeroSection = React.forwardRef<HTMLElement, HeroSectionProps>(
  ({
    ...
    const sectionStyle = useMemo(() => mergeStyles(
      SECTION_BASE,
      SIZE_STYLES[size],
            ...
            <div style={DOTS_ROW}>
              {slides.map((_, index) => (
                <button
                  ...
                  style={mergeStyles(DOT_BASE, currentSlide === index ? DOT_ACTIVE : DOT_INACTIVE)}
            ...
            <div style={LINE_ROW}>
              {slides.map((_, index) => (
                <button
                  ...
                  aria-label={`Go to slide ${index + 1}`}
                >
                  <div style={currentSlide === index ? LINE_FILL_ACTIVE : LINE_FILL_INACTIVE} />
                </button>
              ))}
            </div>
          )
      ...
      >
          ...
          <div key={isSlideMode ? currentSlide : 0}>
            <h1 style={mergeStyles(TITLE_BASE, TITLE_SIZE_STYLES[size])}>
              ...
              {currentContent.subtitle && (
                <span style={mergeStyles(SUBTITLE_BASE, SUBTITLE_SIZE_STYLES[size])}>
                  {currentContent.subtitle}
                </span>
              )}
            </h1>
        ...
        {isSlideMode && showControls && slideCount > 1 && (
          <>
            ...
            >
              <svg style={SVG_ICON} fill="none" stroke="currentColor" viewBox="0 0 24 24">
