 ...
 */
export interface SwitchProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, 'size' | 'className'> {
  variant?: VariantKey
  size?: SizeKey
  error?: boolean
  success?: boolean
  ...
  (
    ...
    const trackStyle = useMemo((): React.CSSProperties => {
      ...
      const stateColor = error
        ? (checked ? STATE_CHECKED_ERROR : STATE_UNCHECKED_ERROR)
        : success
        ? (checked ? STATE_CHECKED_SUCCESS : STATE_UNCHECKED_SUCCESS)
        : undefined
    ...
    }, [variant, size, checked, error, success, isFocused, disabled, dotProp, style])
    ...
    return (
      <div style={WRAPPER_STYLE}>
        <div style={{ position: 'relative', flexShrink: 0 }}>
          ...
          <label htmlFor={switchId} style={{ display: 'block', cursor: disabled ? 'not-allowed' : 'pointer' }}>
