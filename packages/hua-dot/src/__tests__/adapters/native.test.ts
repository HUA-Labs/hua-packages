...
describe("parseTransformString", () => {
  ...
  it("parses skew (keeps string)", () => {
    expect(parseTransformString("skewX(6deg)")).toEqual([{ skewX: "6deg" }]);
    expect(parseTransformString("skewY(12deg)")).toEqual([{ skewY: "12deg" }]);
  });

  it("parses compound transform", () => {
    expect(
      parseTransformString("rotate(45deg) scale(1.1) translateX(16px)"),
    ).toEqual([{ rotate: "45deg" }, { scale: 1.1 }, { translateX: 16 }]);
  });
...
describe("parseBoxShadow", () => {
  ...
  it("skips spread-only ring layer before a real shadow", () => {
    const result = parseBoxShadow(
      "0 0 0 2px var(--color-ring), 0 20px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1)",
    );
    expect(result.shadowColor).toBe("#000000");
    expect(result.shadowOpacity).toBe(0.1);
    expect(result.shadowOffset).toEqual({ width: 0, height: 20 });
    expect(result.shadowRadius).toBe(12.5);
  });

  it("returns empty for spread-only ring layer", () => {
    expect(parseBoxShadow("0 0 0 2px var(--color-ring)")).toEqual({});
  });

  it("skips zero-geometry ring layer before a real shadow", () => {
    const result = parseBoxShadow(
      "0 0 0 0px var(--color-ring), 0 20px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1)",
    );
    expect(result.shadowColor).toBe("#000000");
    expect(result.shadowOpacity).toBe(0.1);
    expect(result.shadowOffset).toEqual({ width: 0, height: 20 });
  ...
  it("returns empty for renderable shadow with CSS variable color", () => {
    expect(parseBoxShadow("0 4px 6px var(--shadow-color)")).toEqual({});
  });
  ...
  it("skips inset shadows", () => {
    expect(parseBoxShadow("inset 0 2px 4px 0 rgb(0 0 0 / 0.05)")).toEqual({});
  });
...
describe("adaptNative — numeric (px→number)", () => {
  ...
  it("converts unitless lineHeight to absolute px (multiplier * fontSize)", () => {
    ...
    expect(adaptNative({ fontSize: "20px", lineHeight: "1.5" })).toEqual({
      fontSize: 20,
      lineHeight: 30,
    });
