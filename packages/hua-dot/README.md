...
- **Flutter recipe subpath — import from '@hua-labs/dot/flutter' for pre-bound dot(), dotMap(), and dotExplain()**
- **Versioned Flutter wire envelope — deterministic JSON-safe recipe plus retained dropped/capability metadata for a future Dart consumer**
- **Flutter recipe wire helpers — createFlutterRecipeWire() and serializeFlutterRecipeWire() from @hua-labs/dot/flutter produce deterministic recipe data**
...
- **getDotAxCatalog() — package-owned read-only AX catalog derived from capability metadata for AI/tooling guidance**
...
// Flutter pre-bound entry + versioned JSON wire data
// import { dot, createFlutterRecipeWire, serializeFlutterRecipeWire }
//   from '@hua-labs/dot/flutter';
// dot('p-4 rounded-lg') → FlutterRecipe
// serializeFlutterRecipeWire(createFlutterRecipeWire('p-4 cursor-pointer'))
// → deterministic hua.dot.flutter.recipe v1 JSON (recipe data, not widgets)
...
| `DOT_AX_CATALOG_SCHEMA_VERSION` | component |  |
| `DOT_AX_CATALOG_SOURCE_PACKAGE` | component |  |
| `DOT_AX_TARGETS` | component |  |
| `DOT_AX_SURFACES` | component |  |
| `dotAxCatalog` | function |  |
| `getDotAxCatalog` | function | getDotAxCatalog() → DotAxCatalog. Read-only package AX catalog derived from CAPABILITY_MATRIX / PROPERTY_TO_FAMILY, with composition metadata for internal marker families such as ring and divide. Stable, JSON-serializable, and intended for AI/tooling guidance without executing dot() or dotExplain(). |
...
| `DotAxCatalog` | type |  |
| `DotAxCatalogEntry` | type |  |
| `DotAxCategory` | type |  |
| `DotAxComposition` | type |  |
| `DotAxCompositionKind` | type |  |
| `DotAxSurface` | type |  |
| `DotAxSurfaceSupport` | type |  |
| `DotAxSurfaceSupportLevel` | type |  |
| `DotAxTarget` | type |  |
| `DotAxTargetSupport` | type |  |
| `DotAxTargetSupportMap` | type |  |
...
[Detailed Guide](./DETAILED_GUIDE.md) — Flutter entry, recipe wire, target boundaries, and advanced style-engine guidance.
