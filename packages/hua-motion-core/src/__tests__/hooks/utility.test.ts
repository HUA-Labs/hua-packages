...
describe("useMotionState", () => {
  ...
  it("should toggle direction when reverse is called", () => {
    ...
    expect(result.current.direction).toBe(
      initialDirection === "forward" ? "reverse" : "forward",
    );
  });
  ...
  it("should calculate remaining time correctly", () => {
    const duration = 1000;
    const { result } = renderHook(() => useMotionState({ duration }));
...
describe("useInView", () => {
  ...
  it("should handle custom root margin formats", () => {
    const { result } = renderHook(() => useInView({ rootMargin: "20px 10px" }));
...
describe("useMouse", () => {
  ...
  it("should accept throttle option", () => {
    const { result } = renderHook(() => useMouse({ throttle: 100 }));
  ...
  it("should accept targetRef option", () => {
    ...
    const { result } = renderHook(() => useMouse({ targetRef }));
  ...
  it("should handle zero throttle", () => {
    const { result } = renderHook(() => useMouse({ throttle: 0 }));
...
describe("useReducedMotion", () => {
  ...
  it("should return a boolean directly", () => {
    const { result } = renderHook(() => useReducedMotion());
...
describe("useWindowSize", () => {
  ...
  it("should accept debounce option", () => {
    const { result } = renderHook(() => useWindowSize({ debounce: 200 }));

    expect(result.current).toBeDefined();
  });

  it("should handle zero debounce", () => {
    const { result } = renderHook(() => useWindowSize({ debounce: 0 }));
