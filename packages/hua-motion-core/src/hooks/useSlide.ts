...
/**
 * useSlide - 통합 슬라이드 모션 훅 (4방향 지원)
 * Unified slide motion hook with 4-direction support
 *
 * @description
 * 스크롤 기반으로 요소를 4방향(up, down, left, right)에서 슬라이드 인 시킴.
 * Slide elements in from 4 directions (up, down, left, right) based on scroll.
 *
 * @example
 * ```tsx
 * const { ref, style } = useSlide({ direction: 'up', distance: 50 })
 * return <div ref={ref} style={style}>Slide Up!</div>
 * ```
 */
export function useSlide<T extends MotionElement = HTMLDivElement>(
