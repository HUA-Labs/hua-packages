...
export function CoreInteraction() {
  const [activeDemo, setActiveDemo] = useState<'hover' | 'click' | 'focus'>('hover')
  ...
  const demos = [
    {
      id: 'hover',
      title: 'useHoverMotion',
      description: '호버 시 모션 효과',
      code: `const motion = useHoverMotion({ 
  scale: 1.1, 
  duration: 200 
})`
    },
    {
      id: 'click',
      title: 'useClickToggle',
      description: '클릭 시 모션 효과',
      code: `const motion = useClickToggle({ 
  scale: 0.95, 
  duration: 150 
})`
    },
    {
      ...
      description: '포커스 시 모션 효과',
      code: `const motion = useFocusToggle({ 
  scale: 1.05, 
  duration: 200 
})`
    }
          ...
          <Panel className="p-6">
            ...
            <div className="flex justify-center">
              <div className="bg-gradient-to-r from-purple-500 to-pink-600 text-white p-8 rounded-lg text-center min-w-[300px]">
                <Icon name="mousePointer" className="w-16 h-16 mx-auto mb-4" />
