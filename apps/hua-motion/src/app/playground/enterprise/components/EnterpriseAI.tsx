...
export function EnterpriseAI() {
  const [activeDemo, setActiveDemo] = useState<'prediction' | 'gestureAI'>('prediction')

  const demos = [
    {
      id: 'prediction',
      title: 'useMotionPrediction',
      description: '머신러닝 기반 모션 예측',
      code: `const prediction = useMotionPrediction({
  model: 'motion-predictor-v1',
  confidence: 0.8,
  onPredict: (motion) => applyMotion(motion)
})`
    },
    {
      id: 'gestureAI',
      title: 'useGestureAI',
      description: 'AI 기반 제스처 인식',
      code: `const gestureAI = useGestureAI({
  camera: true,
  realtime: true,
  gestures: ['wave', 'point', 'grab'],
  onRecognize: (gesture) => handleGesture(gesture)
})`
    }
  ]
        ...
        <div className="space-y-6">
          ...
          <Panel className="p-6">
            <h4 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
              <Icon name="crown" className="w-5 h-5 mr-2 text-yellow-600" />
              Enterprise Only
            </h4>
            
            <div className="flex justify-center">
              <div className="bg-gradient-to-r from-purple-500 to-pink-600 text-white p-8 rounded-lg text-center min-w-[300px]">
