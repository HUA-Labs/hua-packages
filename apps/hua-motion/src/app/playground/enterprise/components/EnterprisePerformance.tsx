...
export function EnterprisePerformance() {
  const [activeDemo, setActiveDemo] = useState<'monitor' | 'optimizer'>('monitor')

  const demos = [
    {
      id: 'monitor',
      title: 'usePerformanceMonitor',
      description: 'FPS 모니터링과 성능 분석',
      code: `const monitor = usePerformanceMonitor({
  targetFPS: 60,
  onDrop: (fps) => console.log('FPS dropped:', fps),
  onRecover: (fps) => console.log('FPS recovered:', fps)
})`
    },
    {
      id: 'optimizer',
      title: 'useMemoryOptimizer',
      description: '메모리 사용량 최적화',
      code: `const optimizer = useMemoryOptimizer({
          ...
          <Panel className="p-6">
            ...
            <div className="flex justify-center">
              <div className="bg-gradient-to-r from-green-500 to-blue-600 text-white p-8 rounded-lg text-center min-w-[300px]">
                <Icon name="gauge" className="w-16 h-16 mx-auto mb-4" />
