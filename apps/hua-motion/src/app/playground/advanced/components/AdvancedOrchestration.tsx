...
export default function AdvancedOrchestration() {
  const [activeDemo, setActiveDemo] = useState<'orchestra' | 'orchestration'>('orchestra')

  const demos = [
    {
      id: 'orchestra',
      title: 'useMotionOrchestra',
      description: '여러 모션을 동시에 제어하는 오케스트라',
      code: `const orchestra = useMotionOrchestra({
  motions: [fadeMotion, slideMotion, scaleMotion],
  sync: true,
  stagger: 200
})`
    },
    {
      id: 'orchestration',
      title: 'useOrchestration',
      description: '복잡한 애니메이션 시퀀스 관리',
      code: `const orchestration = useOrchestration({
  sequences: [
    { motion: fadeIn, delay: 0 },
    { motion: slideUp, delay: 300 },
    { motion: scaleIn, delay: 600 }
  ]
})`
    }
  ]
          ...
          <Panel className="p-6">
            ...
            <div className="flex justify-center">
              <div className="bg-gradient-to-r from-green-500 to-blue-600 text-white p-8 rounded-lg text-center min-w-[300px]">
                <Icon name={"music" as any} className="w-16 h-16 mx-auto mb-4" />
