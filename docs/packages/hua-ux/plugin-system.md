# hua-ux Plugin System

## Overview

The plugin system for hua-ux enables modular extension of the framework through a standardized interface. The plugin infrastructure (interface, registry, defineConfig integration) is fully implemented. Concrete plugin packages are planned for future releases.
...
The plugin system is exported from `@hua-labs/hua-ux/framework`:
...
- `packages/hua-ux/src/framework/plugins/types.ts` - Plugin interface
- `packages/hua-ux/src/framework/plugins/registry.ts` - Plugin registry
- `packages/hua-ux/src/framework/plugins/index.ts` - Exports
