# hua Developer Decision Minimization

## Overview

The core goal of hua is to **reduce the decisions developers have to make every time**. The Preset system handles motion well, but the connection with UI components is lacking. This document covers the plan to fill those gaps.
...
```typescript
// packages/hua/src/presets/product.ts
...
```typescript
// packages/hua/src/presets/marketing.ts
...
```typescript
// packages/hua/src/framework/context/PresetContext.tsx
'use client';
...
import { usePreset } from '@hua-labs/hua/framework';
