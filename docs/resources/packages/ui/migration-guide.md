...
1. **Data / Dashboard** (`@hua-labs/ui/data`)
   ...
   - DataTable (was TransactionsTable), DetailDrawer (was TransactionDetailDrawer)
   - SettlementTimeline, BreakdownCard (was RoutingBreakdownCard), EntityList (was MerchantList)
   - StatsPanel, SectionHeader
   - BarChart, TrendChart, YearlyHeatmap

1a. **Feedback** (`@hua-labs/ui/feedback`) — EmptyState (was DashboardEmptyState)

1b. **Navigation** (`@hua-labs/ui/navigation`) — Sidebar (was DashboardSidebar)

1c. **Interactive** (`@hua-labs/ui/interactive`) — Toolbar (was DashboardToolbar)

1d. **Interactive / Kanban** (`@hua-labs/ui/interactive/kanban`) — KanbanBoard, KanbanColumn, KanbanCard, KanbanProvider, useKanban

> **Note**: `@hua-labs/ui/advanced/dashboard` is deprecated but kept for backwards compatibility.
...
```tsx
import { AdvancedPageTransition, usePageTransition } from "@hua-labs/ui";
```
...
```tsx
import { Button, Input, Card, Modal, Icon, LoadingSpinner } from "@hua-labs/ui";
// 변경 불필요
```
...
```tsx
// 좋은 예: 시맨틱 subpath에서 필요한 것만 import
import { StatCard } from "@hua-labs/ui/data";
import { AdvancedPageTransition } from "@hua-labs/ui/advanced/motion";

// 나쁜 예: StatCard는 @hua-labs/ui/advanced에 없음
...
2. **타입 import**: 타입도 함께 마이그레이션해야 합니다:
   ...
   // After
   import type { StatCardProps } from "@hua-labs/ui/data";
   ```
