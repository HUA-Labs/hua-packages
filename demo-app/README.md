...
`app/api/translations/[language]/[namespace]/route.ts`에서 번역 로딩 로직을 커스터마이징할 수 있습니다.  
You can customize the translation loading logic in `app/api/translations/[language]/[namespace]/route.ts`.
...
### 주요 문제 해결 요약 / Quick Troubleshooting

#### 버튼 스타일이 보이지 않음 / Button styles not visible

**원인**: Tailwind CSS 4가 Button 컴포넌트의 동적 클래스를 제대로 스캔하지 못함  
**Cause**: Tailwind CSS 4 doesn't properly scan dynamic classes in Button component

**해결**: `tailwind.config.js`에 `safelist` 추가  
**Solution**: Add `safelist` to `tailwind.config.js`
...
자세한 내용은 [TROUBLESHOOTING.md](./TROUBLESHOOTING.md#버튼-스타일이-보이지-않는-문제) 참고.
